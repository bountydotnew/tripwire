...
export function getPartToolName(part: MessagePart): string {
  if (part.type === "tool-call")
    return (part as { name?: string }).name ?? "tool"
...
export function parseErrorMessage(message: string): {
  ...
  if (
    ...
    return {
      ...
      detail:
        "The AI service isn't configured. Check that OPENROUTER_API_KEY is set in your environment.",
    }
  }
  ...
  if (lowerMsg.includes("repository") || lowerMsg.includes("repoid")) {
    return {
      ...
      detail:
        "No active repository was resolved for this chat request. Open Integrations and confirm at least one repository is connected.",
    }
  }
...
export function parseActionResult(content: string): ActionResultData | null {
  try {
    ...
    if (
      parsed?.root === "main" &&
      parsed?.elements?.main?.type === "ActionResult"
    ) {
  ...
  switch (action) {
    case "add_to_blacklist":
      return {
        prefix: "Blacklist",
        suffix: "",
        consequence: "They will be blocked from all future contributions.",
        buttonLabel: "blacklist",
      }
    case "remove_from_blacklist":
      return {
        prefix: "Remove",
        suffix: "from the blacklist",
        consequence: null,
        buttonLabel: "remove",
      }
    case "add_to_whitelist":
      return {
        prefix: "Whitelist",
        suffix: "",
        consequence: "They will bypass all rule checks.",
        buttonLabel: "whitelist",
      }
    case "remove_from_whitelist":
      return {
        prefix: "Remove",
        suffix: "from the whitelist",
        consequence: null,
        buttonLabel: "remove",
      }
    case "move_to_whitelist":
      return {
        prefix: "Move",
        suffix: "to the whitelist",
        consequence: "They will be unblocked and bypass all rule checks.",
        buttonLabel: "move",
      }
    case "move_to_blacklist":
      return {
        prefix: "Move",
        suffix: "to the blacklist",
        consequence: "They will be blocked from all future contributions.",
        buttonLabel: "move",
      }
    default:
