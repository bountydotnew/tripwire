 ...
 *   - "read"     : call /api/tools/run for a directInvokable tool and inject
 *                  the resulting render spec as an assistant message
...
export interface SlashCommand {
	...
	/** For `read`: the tool name to hit via /api/tools/run. */
	tool?: string;
	...
	// ─── Mutations ───────────────────────────────────────────────────
	{
		...
		buildConfirm: (raw) => {
			...
			return {
				...
				description: "All future PRs, issues, and comments from this user will be blocked.",
	...
	},
	{
		...
		buildConfirm: (raw) => {
			...
			return {
				...
				description: "Their future contributions will go through the normal rule pipeline again.",
				confirmLabel: "Remove",
				danger: false,
			};
		},
	...
	},
	{
		...
		buildConfirm: (raw) => {
			...
			return {
				...
				description: "They will start going through the normal rule pipeline again.",
				confirmLabel: "Remove",
				danger: false,
			};
		},
...
export function filterCommands(query: string): SlashCommand[] {
	...
	// Once the user has typed past the command name and started args,
	// only show the exact matching command (so the dropdown gets out of the way).
