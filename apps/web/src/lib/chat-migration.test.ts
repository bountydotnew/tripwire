...
import { mergeMessagesPreservingResults } from "./chat-persistence";
import { parseActionResult } from "./chat-format";
...
describe("AI SDK chat migration helpers", () => {
				...
				parts: [
					{
						...
						input: { username: "octo" },
						output: { root: "main", elements: { main: { type: "ContributorScore", props: {} } } },
					},
				],
			},
		];
	...
	it("preserves server-side tool outputs by message id when message order shifts", () => {
		...
		const merged = mergeMessagesPreservingResults(staleClient, stored) as any[];
		const mergedAssistant = merged.find((message) => message.id === "assistant-1");
