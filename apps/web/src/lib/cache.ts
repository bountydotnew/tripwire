...
export function invalidateGitHubUser(queryClient: QueryClient, username: string) {
	queryClient.invalidateQueries({ queryKey: ["github", "user", username.toLowerCase()] });
	queryClient.invalidateQueries({ queryKey: ["github", "repos", username.toLowerCase()] });
	queryClient.invalidateQueries({ queryKey: ["github", "profile", username.toLowerCase()] });
	queryClient.invalidateQueries({ queryKey: ["github", "contributions", username.toLowerCase()] });
...
export function invalidateEventCaches(queryClient: QueryClient, repoId: string) {
	queryClient.invalidateQueries({
		predicate: (query) => {
			const key = query.queryKey;
			const str = JSON.stringify(key);
			return str.includes(repoId) && (
				str.includes("events") ||
				str.includes("digest") ||
