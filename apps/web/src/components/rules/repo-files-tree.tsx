...
function RenderedMarkdown({ content }: { content: string }) {
	...
	return (
		<div
			className="prose-file-preview p-3 text-[12px] leading-[1.6] text-tw-text-secondary overflow-auto h-full"
...
interface RepoFilesTreeProps {
	...
	generateAgentsMd: (config: RuleConfig, repoFullName: string, honeypotPhrase?: string) => string;
	onUpdateContent: (kind: "rules-md" | "pr-template" | "agents-md", content: string) => void;
	onToggle: (path: string, value: boolean) => void;
	onAddHoneypotPhrase: (target: "prTemplate" | "agentsMd", kind: HoneypotPhraseKind) => void;
	onRemoveHoneypotPhrase: (target: "prTemplate" | "agentsMd", index: number) => void;
}
...
const FILE_DESCRIPTIONS: Record<FileKey, string> = {
	"RULES.md": "Human-readable summary of your enabled rules, committed to the repo root.",
	"PULL_REQUEST_TEMPLATE.md": "Pre-fills every PR description with a checklist tied to your rules.",
	"AGENTS.md": "Instructions for AI coding agents. Contains hidden honeypot phrases that bots tend to include in their PRs.",
};
...
function getFileContent(
	...
	if (file === "PULL_REQUEST_TEMPLATE.md") {
		...
		const showHoneypot = section.honeypotEnabled && section.honeypotPhrases.length > 0;
	...
	const showHoneypot = section.honeypotEnabled && section.honeypotPhrases.length > 0;
	const phrase = showHoneypot ? section.honeypotPhrases[0].phrase : undefined;
	const generated = props.generateAgentsMd(props.config, props.repoFullName, phrase);
	const custom = section.customContent;
	return { value: custom.length > 0 ? custom : generated, generated };
}

function getContentKind(file: FileKey): "rules-md" | "pr-template" | "agents-md" {
...
function getAutoSync(file: FileKey, config: RuleConfig): boolean {
	...
	if (file === "PULL_REQUEST_TEMPLATE.md") return config.repoFiles.prTemplate.autoSync;
...
function HoneypotSection({
	...
	isPending: boolean;
	onToggle: (path: string, value: boolean) => void;
	onAddPhrase: (target: "prTemplate" | "agentsMd", kind: HoneypotPhraseKind) => void;
	...
	return (
		<div className="flex flex-col gap-2">
			<label className="flex items-start gap-2 cursor-pointer select-none -mx-1 px-1 py-0.5 rounded hover:bg-[#ffffff08]">
				<Checkbox
				...
				<span className="flex flex-col gap-0.5">
					...
					<span className="text-[11px] text-[#FFFFFF73] leading-snug">
						Hidden instruction that AI agents follow. Pair with the <span className="text-tw-accent">AI honeypot</span> rule to catch them.
					</span>
				</span>
			</label>
			{section.honeypotEnabled && (
				<div className="ml-5 flex flex-col gap-1.5">
					...
					<div className="flex items-center gap-1.5 flex-wrap pt-0.5">
						<span className="text-[11px] text-[#FFFFFF73] mr-1">Add:</span>
...
function FileSettings({
	...
	return (
		<div className="rounded-xl bg-tw-card border border-tw-border-card p-4 flex flex-col gap-3">
			...
			<label className="flex items-center gap-2 text-[13px] text-[#FFFFFFCC] cursor-pointer select-none -mx-1 px-1 py-0.5 rounded hover:bg-[#ffffff08]">
				<Checkbox
					checked={autoSync}
					onCheckedChange={(value) => onToggle(getAutoSyncKey(file), value === true)}
				/>
	...
	return (
		<div className="flex flex-col gap-3">
			...
			<div className="flex gap-0 rounded-xl overflow-hidden border border-tw-border-card min-h-[360px]">
				...
				{/* File content editor */}
				<div className="flex-1 min-w-0 bg-tw-card flex flex-col">
					{activeFile && fileContent ? (
						<>
							<div className="flex items-center justify-between px-3 py-1.5 border-b border-tw-border-card">
								<div className="flex items-center gap-2">
									<span className="text-[12px] font-mono text-tw-text-secondary">
										{getTargetPath(activeFile)}
									</span>
									{getAutoSync(activeFile, props.config) && (
										<span className="text-[10px] text-tw-accent bg-tw-accent/10 px-1.5 py-0.5 rounded">
											synced
										</span>
									)}
								</div>
									...
									{(["preview", "edit"] as const).map((m) => (
										<button
											...
											className={`px-2 py-1 text-[11px] rounded transition-colors cursor-pointer ${
												editorMode === m
													? "text-tw-text-primary bg-tw-inner"
							...
							<div className="flex-1 min-h-0 overflow-auto">
								{editorMode === "preview" ? (
									...
									<textarea
										...
										spellCheck={false}
										className="w-full h-full font-mono text-[11px] leading-snug text-tw-text-secondary bg-transparent p-3 outline-none resize-none"
						...
						<div className="h-full flex items-center justify-center text-[13px] text-tw-text-tertiary">
