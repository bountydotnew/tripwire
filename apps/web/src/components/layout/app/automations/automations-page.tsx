...
} from "#/constants/automation-templates"
import { useTRPC } from "#/integrations/trpc/react"
import { useWorkspace } from "#/providers/workspace-context"

const routeApi = getRouteApi("/_app/$orgHandle/automations/")

export function AutomationsPageSkeleton() {
  return (
    <div className="mx-auto max-w-3xl p-6">
      <div className="mb-4 h-6 w-32 animate-pulse rounded bg-white/5" />
      <div className="mb-5 h-9 w-44 animate-pulse rounded-[10px] bg-white/5" />
      <div className="flex flex-col gap-2">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="h-[68px] w-full animate-pulse rounded-xl bg-white/5"
          />
        ))}
      </div>
    </div>
