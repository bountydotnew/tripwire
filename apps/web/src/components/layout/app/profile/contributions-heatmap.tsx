...
import {
  ...
  type HeatmapCell,
} from "@tripwire/ui/icons/contributions-heatmap-canvas"
...
const TOOLTIP_FMT = new Intl.DateTimeFormat(undefined, {
  month: "short",
  day: "numeric",
  year: "numeric",
})
...
export function ContributionsHeatmap({
  ...
  const daysByWeek = useMemo(
    () =>
      data.weeks.map((week) => {
        const arr: ({ date: string; count: number } | null)[] =
          Array(7).fill(null)
  ...
  return (
    <>
          ...
          >
            <div className="w-max">
              ...
              <div
                ...
                style={{ marginLeft: gutterW + labelGap, width }}
              >
      ...
      <AnimatePresence>
        {hovered && (
          <motion.div
            className="pointer-events-none fixed z-50 rounded-md border border-tw-border bg-tw-card px-2 py-1 text-[11px] whitespace-nowrap text-tw-text-primary"
