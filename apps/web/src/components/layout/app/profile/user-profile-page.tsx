...
import { CommunitySignals } from "#/components/layout/app/profile/community-signals"
import { ContributionsHeatmap } from "#/components/layout/app/profile/contributions-heatmap"
import { PinnedRepos } from "#/components/layout/app/profile/pinned-repos"
 ...
 * loader+head shell. `getRouteApi` gives us the typed param access
 * without re-importing the Route export (which would create a
 * circular import).
 */
const routeApi = getRouteApi("/_app/users/$username")
...
export function UserProfilePage() {
  ...
  const scoreQueryOpts = trpc.reputation.getScore.queryOptions({
    repoId: repo?.id ?? "",
    username,
  })
  const profileQueryOpts = trpc.reputation.getProfile.queryOptions({
    repoId: repo?.id ?? "",
    username,
  })
  ...
  // Webhook for this user → bump `user:USERNAME` signal → SSE/poll
