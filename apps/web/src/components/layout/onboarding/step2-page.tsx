...
export function Step2Page() {
  ...
  // User likely just clicked through to GitHub to install the App on a
  // new org/repo. When they tab back, refresh `orgs.myRepos` so the new
  // selection appears without forcing them to navigate.
  const refresh = useCallback(async () => {
    await queryClient.invalidateQueries({
      queryKey: trpc.orgs.myRepos.queryKey(),
    })
  }, [queryClient, trpc.orgs.myRepos])
    ...
    <div className="flex w-full flex-col gap-6">
      ...
      <div className="flex items-center justify-end">
        <Button
          variant="default"
          size="sm"
          render={<a href={routes.api.githubInstall}>Install GitHub App</a>}
        />
      </div>
    </div>
  )
