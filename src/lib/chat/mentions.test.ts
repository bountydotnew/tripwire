...
const blacklisted: ListedUserMention[] = [
	{ username: "octocat", status: "blacklisted", avatarUrl: "https://github.com/octocat.png" },
];

const whitelisted: ListedUserMention[] = [
	{ username: "hubot", status: "whitelisted", avatarUrl: "https://github.com/hubot.png" },
	{ username: "mona", status: "whitelisted", avatarUrl: "https://github.com/mona.png" },
];
...
describe("mention helpers", () => {
	...
	it("filters listed users and skips selected usernames", () => {
		expect(buildListedUserSuggestions(blacklisted, whitelisted, "mo", ["octocat"]))
			.toEqual([{ username: "mona", status: "whitelisted", avatarUrl: "https://github.com/mona.png" }]);
	});
	...
	it("removes the typed trigger after selecting a chip", () => {
		...
		expect(replaceMentionTrigger("please check @hu", trigger!)).toBe("please check");
	});

	it("serializes mention chips before the freeform message", () => {
		expect(composeMentionMessage([whitelisted[0]], "what changed?"))
			.toBe("@hubot what changed?");
	});
});
