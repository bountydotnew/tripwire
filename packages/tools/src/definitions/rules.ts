...
const getRepoRules = defineTool({
	...
	chatRender: (config) => {
		...
		const rules = entries
			...
			.map(([id, rule]) => {
				const r = rule as { enabled: boolean; action: string } & Record<string, unknown>;
...
function getRuleDetail(
	...
	if (ruleId === "languageRequirement" && rule.language) return `${rule.language}`;
	if (ruleId === "minMergedPrs" && rule.count != null) return `${rule.count} PRs`;
	if (ruleId === "accountAge" && rule.days != null) return `${rule.days} days`;
	if (ruleId === "maxPrsPerDay" && rule.limit != null) return `${rule.limit}/day`;
	if (ruleId === "maxFilesChanged" && rule.limit != null) return `${rule.limit} files`;
