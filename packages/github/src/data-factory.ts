...
function transformSearchItemToPR(item: Record<string, unknown>): CachedPR {
	...
	if (mergedAt && createdAt) {
		timeToMergeMinutes = Math.round((new Date(mergedAt).getTime() - new Date(createdAt).getTime()) / 60_000);
	}
	...
	return {
		...
		mergedAt,
		repoFullName,
		labels: labels.map((l) => ({ name: (l.name as string) ?? "", color: (l.color as string) ?? "6E6E6E" })),
...
async function enrichPRWithDetails(token: string, pr: CachedPR): Promise<CachedPR> {
	try {
		const [owner, repo] = pr.repoFullName.split("/");
		if (!owner || !repo) return pr;
		const detail = await githubApi(`/repos/${owner}/${repo}/pulls/${pr.number}`, token);
		if (!detail) return pr;
		...
		const authorLogin = ((detail.user as Record<string, unknown>)?.login as string) ?? pr.authorLogin;
		const closerLogin = (mergedByUser?.login as string) ?? (closedByUser?.login as string) ?? null;
		...
		return {
			...
			headSha: (detail.merge_commit_sha as string) ?? (detail.head as Record<string, unknown>)?.sha as string ?? null,
			body: ((detail.body as string) ?? "").slice(0, 500) || null,
			closedBy: closerLogin,
			selfClosed: closerLogin ? closerLogin.toLowerCase() === authorLogin.toLowerCase() : null,
		};
	} catch {
		return pr;
	}
...
export async function fetchUserPRs(
	...
	const enriched = await Promise.all(toEnrich.map((pr) => enrichPRWithDetails(token, pr)));
...
export async function fetchComments(
	...
	const [issueComments, reviewComments] = await Promise.all([
		githubApi(`/repos/${owner}/${repo}/issues/${issueNumber}/comments?per_page=100`, token).catch(() => []),
		githubApi(`/repos/${owner}/${repo}/pulls/${issueNumber}/comments?per_page=100`, token).catch(() => []),
	]);
...
export async function fetchPRDetail(
	...
	const [prData, filesData, reviewsData, commitsData, issueComments, reviewComments] = await Promise.all([
		githubApi(`/repos/${owner}/${repo}/pulls/${prNumber}`, token),
		githubApi(`/repos/${owner}/${repo}/pulls/${prNumber}/files?per_page=100`, token).catch(() => []),
		githubApi(`/repos/${owner}/${repo}/pulls/${prNumber}/reviews`, token).catch(() => []),
		githubApi(`/repos/${owner}/${repo}/pulls/${prNumber}/commits?per_page=100`, token).catch(() => []),
		githubApi(`/repos/${owner}/${repo}/issues/${prNumber}/comments?per_page=100`, token).catch(() => []),
		githubApi(`/repos/${owner}/${repo}/pulls/${prNumber}/comments?per_page=100`, token).catch(() => []),
	]);
	...
	const reviewerMap = new Map<string, { login: string; state: string; avatarUrl: string }>();
	...
	return { pr, files, reviewers: Array.from(reviewerMap.values()), commitMessages, comments };
}
