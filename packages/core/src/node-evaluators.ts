...
register("rule", "accountAge", {
  ...
  evaluate(data, ctx) {
    const threshold =
      ((data.params as Record<string, unknown>)?.days as number) ?? 30
...
register("rule", "minMergedPrs", {
  ...
  evaluate(data, ctx) {
    const threshold =
      ((data.params as Record<string, unknown>)?.count as number) ?? 15
...
register("rule", "repoActivityMinimum", {
  ...
  evaluate(data, ctx) {
    const threshold =
      ((data.params as Record<string, unknown>)?.minRepos as number) ?? 3
...
register("rule", "maxPrsPerDay", {
  ...
  evaluate(data, ctx) {
    const limit =
      ((data.params as Record<string, unknown>)?.limit as number) ?? 5
...
register("rule", "maxFilesChanged", {
  ...
  evaluate(data, ctx) {
    const limit =
      ((data.params as Record<string, unknown>)?.limit as number) ?? 20
...
register("rule", "language", {
  ...
  evaluate(data, ctx) {
    ...
    const scriptChecks: Record<string, RegExp> = {
      ...
      ar: arabicPattern,
      Arab: arabicPattern,
      hi: devanagariPattern,
      Deva: devanagariPattern,
    }
    ...
    return {
      pass,
      detail: `${pass ? "PASS" : "FAIL"} -- content ${pass ? "matches" : "does not match"} language: ${code}`,
    }
  },
})
...
register("rule", "contributorScore", {
  ...
  evaluate(data, ctx) {
    const threshold =
      ((data.params as Record<string, unknown>)?.minScore as number) ?? 50
