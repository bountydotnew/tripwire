...
describe("filter-pipeline rule checks", () => {
  ...
  describe("maxPrsPerDay", () => {
    it("should count PRs opened today correctly", async () => {
      ...
      const count = await githubApi.countUserPrsToday(
        "token",
        "testuser",
        "owner/repo"
      )
      expect(count).toBe(3)
      expect(mockCountUserPrsToday).toHaveBeenCalledWith(
        "token",
        "testuser",
        "owner/repo"
      )
  ...
  describe("maxFilesChanged", () => {
    it("should get file count for PR", async () => {
      ...
      const count = await githubApi.getPrFilesCount(
        "token",
        "owner",
        "repo",
        123
      ...
      expect(mockGetPrFilesCount).toHaveBeenCalledWith(
        "token",
        "owner",
        "repo",
        123
  ...
  describe("repoActivityMinimum", () => {
    it("should get public repo count", async () => {
      const mockGetUserPublicRepoCount = vi.mocked(
        githubApi.getUserPublicRepoCount
      )
      mockGetUserPublicRepoCount.mockResolvedValue(12)

      const count = await githubApi.getUserPublicRepoCount(
        "token",
        "activeuser"
      )
      expect(count).toBe(12)
      expect(mockGetUserPublicRepoCount).toHaveBeenCalledWith(
        "token",
        "activeuser"
      )
    })
  ...
  describe("requireProfileReadme", () => {
    it("should return true when user has profile README", async () => {
      ...
      const hasReadme = await githubApi.hasProfileReadme(
        "token",
        "userWithReadme"
      )
      expect(hasReadme).toBe(true)
      expect(mockHasProfileReadme).toHaveBeenCalledWith(
        "token",
        "userWithReadme"
      )
    })
...
describe("rule config defaults", () => {
  it("should have correct default values for new rules", async () => {
    ...
    expect(DEFAULT_RULE_CONFIG.maxFilesChanged).toEqual({
      enabled: false,
      action: "block",
      limit: 20,
    })
    expect(DEFAULT_RULE_CONFIG.repoActivityMinimum).toEqual({
      enabled: false,
      action: "block",
      minRepos: 3,
    })
    expect(DEFAULT_RULE_CONFIG.requireProfileReadme).toEqual({
      enabled: false,
      action: "block",
    })
  })
