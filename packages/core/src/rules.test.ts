...
describe("languageRequirement", () => {
  ...
  it("passes English text when English required", () => {
    expect(
      isLikelyLanguage(
        "This is a pull request that fixes the authentication bug in the login flow.",
        "English"
      )
    ).toBe(true)
  })
  ...
  it("fails non-English text when English required", () => {
    ...
    expect(
      isLikelyLanguage(
        "这是 一个 修复 登录 流程 中的 认证 错误 的 拉取请求",
        "English"
      )
    ).toBe(false)
  })
  ...
  it("passes Chinese text when Chinese required", () => {
    expect(
      isLikelyLanguage(
        "这是一个修复登录流程中的认证错误的拉取请求。更改了认证模块的逻辑。这是很重要的改进。",
        "Chinese"
      )
    ).toBe(true)
  })
  ...
  it("passes English text with code mixed in", () => {
    const mixed =
      "Fix the auth bug here. This resolves the issue with expired sessions. We should test this carefully before merging."
  ...
  it("passes unknown languages (graceful fallback)", () => {
    expect(
      isLikelyLanguage(
        "Anything here really doesnt matter for this test case",
        "Klingon"
      )
    ).toBe(true)
  })
...
describe("cryptoAddressDetection", () => {
  it("detects Bitcoin legacy address", () => {
    const result = detectCryptoAddress(
  ...
  it("detects Solana address", () => {
    const result = detectCryptoAddress(
      "7EcDhSYGxXyscszYEp35KHN8vvw3svAuLKTzXwCFLtV5"
    )
    expect(result).not.toBeNull()
  ...
  it("does NOT flag normal text", () => {
    expect(
      detectCryptoAddress("This PR fixes a bug in the payment processor.")
    ).toBeNull()
  ...
  it("does NOT flag git commit hashes (40 hex but no 0x prefix)", () => {
    expect(
      detectCryptoAddress("commit a1b2c3d4e5f6789012345678901234567890abcd")
    ).toBeNull()
  })
})
...
describe("ruleApplies (scope resolution)", () => {
  ...
  it("disabled rule never applies", () => {
    expect(
      ruleApplies({ enabled: false }, "pull_request", {
        pullRequests: true,
        issues: true,
        comments: true,
      })
  ...
  it("enabled rule uses global scope", () => {
    expect(
      ruleApplies({ enabled: true }, "pull_request", {
        pullRequests: true,
        issues: false,
        comments: false,
      })
    ...
    expect(
      ruleApplies({ enabled: true }, "issue", {
        pullRequests: true,
        issues: false,
        comments: false,
      })
  ...
  it("unknown content type always runs", () => {
    expect(
      ruleApplies({ enabled: true }, undefined, {
        pullRequests: false,
        issues: false,
        comments: false,
      })
