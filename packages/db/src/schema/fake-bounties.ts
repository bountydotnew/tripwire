 ...
 */
export const fakeBountyConfigs = pgTable(
	"fake_bounty_configs",
	{
		...
		declineMessage: text("decline_message").notNull().default(
		...
		issueLabels: jsonb("issue_labels").$type<string[]>().notNull().default(["bounty", "good first issue", "help wanted"]),
...
export const fakeBounties = pgTable(
	"fake_bounties",
	{
		...
		status: text("status").$type<FakeBountyStatus>().notNull().default("active"),
	...
	(t) => [
		...
		uniqueIndex("fake_bounties_repo_issue_uniq").on(t.repoId, t.githubIssueNumber),
	],
);
